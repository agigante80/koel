...
| `SYNC_LOG_KEEP` | How many sync logs to keep under `storage/logs/`. A scan with nothing to report writes none; older logs beyond this count are removed after each write. `0` keeps them all. | `30` |
